    ...
    return (
        <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
            ...
            <div className="text-danger fs-4 wd-name">
                {!editing && (
                    <FaPencil onClick={() => {setEditing(true); setRole(user.role); setEmail(user.email);
            ...
            <span className="wd-email">
                {!editing && (
                    <div className="wd-name"
                         onClick={() => setEditing(true)}>{user.email}</div>)}
                {user && editing && (
                    <input className="form-control w-50 wd-edit-name"
            ...
            <span className="wd-roles">
                {!editing && (
                    <div className="wd-name"
                         onClick={() => setEditing(true)}>{user.role}</div>)}
                {user && editing && (
                    <select value={role} onChange={(e) => setRole(e.target.value)}
                            className="form-select flost-start w-25 wd-select-role"
                            onKeyDown={(e) => {
                                if (e.key === "Enter") {saveUser();}}}>
